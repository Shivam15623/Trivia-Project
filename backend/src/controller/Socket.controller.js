...
async function handleTimerExpiry(sessionCode, generation) {
  ...
  // ── 3. Atomic claim — replaces old .some() idempotency check ───
  ...
  // ✅ REST handler already claimed — nothing to do
  if (!claimed) {
    console.log(
      `[timer] question already claimed by submit, expiry no-op session=${sessionCode}`,
    );
    return;
  }

  // ── 4. Record miss ─────────────────────────────────────────────
  // ✅ No need to set questionEntry.used — already done atomically
